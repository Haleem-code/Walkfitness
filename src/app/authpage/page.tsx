...
const AuthPageWithStreak = () => {
  ...
  const checkStreak = useCallback(async () => {
    try {
      ...
      if (response.ok) {
        ...
        if (data.hasStreak && data.xpAwarded > 0) {
          setShowStreakNotification(true)
        } else {
          // If no streak reward, redirect immediately
          setTimeout(() => {
            router.push("/walk")
          }, 1000)
        }
      } else {
        console.error("Streak check failed:", await response.json())
        // Redirect anyway if streak check fails
        setTimeout(() => {
          router.push("/walk")
        }, 1000)
      }
  ...
  useEffect(() => {
    if (status === "authenticated") {
      // Check for streak when user is authenticated
      checkStreak()
    }
  }, [status, checkStreak])
  ...
  const handleStreakNotificationClose = () => {
    ...
    // Redirect to walk page after closing notification
    setTimeout(() => {
      router.push("/walk")
    }, 500)
  }
    ...
    <>
      ...
      {streakData && (
        <StreakNotification
          ...
          xpAwarded={streakData.xpAwarded}
...
export default AuthPageWithStreak