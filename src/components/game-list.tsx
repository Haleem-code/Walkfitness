...
interface GamesListProps {
  ...
  joinedGameIds?: string[] // Add this prop to track joined games
}

export default function GamesList({ games, type, joinedGameIds = [] }: GamesListProps) {
  const router = useRouter()

  const handleJoinGame = async (gameId: string) => {
    const isJoined = joinedGameIds.includes(gameId)
    
    if (isJoined) {
      // If already joined, navigate directly to the game
  ...
  // Helper function to get button text and style
  const getButtonProps = (game: Game) => {
    ...
    if (isJoined) {
      return {
        text: "Enter",
        className: "bg-gradient-to-r from-purple-500 to-purple-600 hover:from-purple-600 hover:to-purple-700 text-white font-semibold px-3 py-1 text-xs rounded-lg transition-all duration-300 hover:scale-105 shadow-lg",
        disabled: false
      }
    } else if (isFull) {
      return {
        text: "Full",
        className: "bg-gray-600 text-gray-400 font-semibold px-3 py-1 text-xs rounded-lg cursor-not-allowed opacity-60",
        disabled: true
      }
    } else {
      return {
        text: "Join",
        className: "bg-purple-400 hover:bg-purple-500 text-white font-semibold px-3 py-1 text-xs rounded-lg transition-all duration-300 hover:scale-105 shadow-lg",
        disabled: false
      }
    }
  }
  ...
  return (
    <div className="space-y-3">
      ...
      {/* Games */}
      {games.map((game) => {
        const buttonProps = getButtonProps(game)
              ...
              <div className="md:hidden space-y-3">
                ...
                <div className="flex items-center justify-between gap-3">
                  ...
                  <Button
                    ...
                    onClick={() => handleJoinGame(game.id)}
                    disabled={buttonProps.disabled}
                    className={`${buttonProps.className} px-4 py-2 h-10 flex-shrink-0`}
                  >
                    {buttonProps.text}
                  </Button>
                </div>
                ...
                <div className="grid grid-cols-4 gap-2 text-xs">
                    ...
                    <div className="flex items-center justify-center gap-1">
                      <div className="flex -space-x-1">
                        {[...Array(Math.min(2, game.players))].map((_, i) => (
                          <div
                            key={i}
                            className="w-4 h-4 bg-gradient-to-br bg-purple-400 rounded-full border border-gray-800"
                          />
                        ))}
                      </div>
                  ...
                  <div className="text-center">
                    ...
                    <div>
                      {type === "sponsored" ? (
                        <Badge className="bg-purple-400 text-black text-xs px-2 py-0.5 font-semibold">FREE</Badge>
                      ) : (
